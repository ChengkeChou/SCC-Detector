...
class CellSegmentationInference:
    ...
    def load_model(self):
        ...
        try:
            ...
            if self.model_type == "yolov8":
            ...
            elif self.model_type == "dino":                # 使用DINO模型
                from models.cell_segmentation import CellSegmentationModel                
                self.model = CellSegmentationModel(
                    model_path=self.model_path,
                    model_type="dino",
                ...
                # 使用混合模型
                self.model = HybridCellSegmentationModel(self.num_classes, device=self.device)
                ...
                try:
                    ...
                    if os.path.exists(class_map_path):
                        with open(class_map_path, 'r', encoding='utf-8') as f:
                            ...
                            logger.info(f"已加载用户类别映射: {self.class_map}")
                    
                    logger.info("已加载混合细胞分割模型 (Hybrid)")
                except Exception as e:
                    logger.error(f"加载混合模型时出错: {e}")
                    raise
    ...
    def predict(self, image):
        ...
        if self.model_type in ["yolov8", "cellpose", "dino"]:
            ...
            # 组装预测结果
            prediction = {                'boxes': outputs[0]['boxes'][keep],
            ...
            # 执行推理
            logger.info(f"使用混合模型进行推理，图像尺寸: {original_image.shape}")
            with torch.no_grad():
                prediction = self.model.predict(image_tensor, self.confidence_threshold)
                
            # 应用用户类别映射（如果有）
            if hasattr(self, 'class_map') and prediction and 'labels' in prediction:
                logger.info(f"应用用户类别映射: {self.class_map}")
                if isinstance(prediction['labels'], torch.Tensor) and len(prediction['labels']) > 0:
                    mapped_labels = []
                    for label in prediction['labels'].cpu().numpy():
                        label_str = str(int(label))
                        if label_str in self.class_map:
                            mapped_labels.append(int(self.class_map[label_str]))
                        else:
                            mapped_labels.append(int(label))
                    prediction['labels'] = torch.tensor(mapped_labels, device=self.device)
                    logger.info(f"已将标签映射为用户类别")
        else:
            # 默认混合模型
            transform = transforms.Compose([
                transforms.ToTensor(),
