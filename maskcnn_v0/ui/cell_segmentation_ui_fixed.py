...
class CellSegmentationUI(QMainWindow):
    ...
    def setup_inference_tab(self, tab):
        ...
        self.model_type_combo.setCurrentText("hybrid")
    ...
    def scan_models(self):
        ...
        if os.path.exists(output_dir):
            ...
            if os.path.exists(best_model):
                ...
                logger.info(f"找到训练好的混合模型: {best_model}")
        ...
        for user_dir in user_data_dirs:
            if os.path.exists(user_dir):
                logger.info(f"扫描用户数据目录: {user_dir}")
                for root, dirs, files in os.walk(user_dir):
                    for file in files:
                        if file.endswith('.pth'):
                            model_path = os.path.join(root, file)
                            if "hybrid" in model_path.lower() or "best_model" in model_path.lower():
                                self.model_combo.insertItem(0, model_path)
                                logger.info(f"找到用户训练的混合模型: {model_path}")
                            else:
                                self.model_combo.addItem(model_path)
                                logger.info(f"找到用户训练模型: {model_path}")
