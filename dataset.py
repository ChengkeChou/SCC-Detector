...
def _load_from_processed(self):
        ...
        # 遍历每个类别文件夹
        for class_dir in self.processed_dir.glob("im_*"):
            if not class_dir.is_dir():
                continue

            class_name_from_dir = class_dir.name  # e.g., "im_Parabasal"
            class_name = class_name_from_dir.replace("im_", "")
            if class_name not in class_mapping:
                print(f"警告: 未知类别 {class_name}，跳过目录 {class_dir}")
                continue
            ...
            for img_file in class_dir.glob("**/*.png"):
                try:
                    ...
                    if relative_path.parts and relative_path.parts[0] == class_name_from_dir:
                        # 跳过重复路径
                        continue
                except ValueError:
                    pass

                if "_mask" not in img_file.name:  # 排除掩码文件
                    img_path = img_file.resolve().absolute()

                    if not os.path.isfile(str(img_path)):
                        print(f"警告: 文件不存在或无法读取: {img_path}")
                        continue
                    ...
                    try:
                        ...
                        self.boxes.append([[0, 0, w, h, class_label]])
